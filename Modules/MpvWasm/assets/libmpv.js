...
["___indirect_function_table","_ff_h264_cabac_tables","___em_lib_deps_sdlaudio","___em_lib_deps_sdlcamera","___em_lib_deps_sdlmouse","__emscripten_proxy_main","_main","onRuntimeInitialized"].forEach((prop) => {
...
// include: /tmp/tmpig92irnd.js
  ...
  (() => {
    ...
    function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'libmpv.data';
      var REMOTE_PACKAGE_BASE = 'libmpv.data';
...
Module['FS_createPath']("/", "shaders", true, true);
      ...
      function processPackageData(arrayBuffer) {
        ...
        // Reuse the bytearray from the XHR as the source for file reads.
          ...
          }          Module['removeRunDependency']('datafile_libmpv.data');

      };
      Module['addRunDependency']('datafile_libmpv.data');
    ...
    loadPackage({"files": [{"filename": "/shaders/.keep", "start": 0, "end": 0}], "remote_package_size": 0});

  })();

// end include: /tmp/tmpig92irnd.js
// include: /tmp/tmpmf2xyf8w.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if (Module['$ww'] || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpmf2xyf8w.js
// include: /tmp/tmpg15glc_r.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmpg15glc_r.js
...
if (ENVIRONMENT_IS_PTHREAD) {
  ...
  function handleMessage(e) {
    try {
      ...
      if (cmd === 'load') { // Preload command that is called once per worker to parse and load the Emscripten code.
      ...
      } else if (cmd === 'networkfsMount') {
        try {
          var mounted = NetworkFS.mountFile(msgData['path'], msgData['url'], msgData['opts'] || {});
          postMessage(JSON.stringify({ type: 'networkfs-mounted', path: mounted.path, size: mounted.size, backendPointer: mounted.backendPointer }));
        } catch (ex) {
          postMessage(JSON.stringify({ type: 'networkfs-error', path: msgData['path'], error: String(ex && (ex.stack || ex.message) || ex) }));
        }
      } else if (cmd === 'networkfsAttach') {
        try {
          var attached = NetworkFS.attachFile(msgData['path'], msgData['url'], msgData['opts'] || {});
          postMessage(JSON.stringify({ type: 'networkfs-attached', path: attached.path, size: attached.size, backendPointer: attached.backendPointer }));
        } catch (ex) {
          postMessage(JSON.stringify({ type: 'networkfs-error', path: msgData['path'], error: String(ex && (ex.stack || ex.message) || ex) }));
        }
      } else if (cmd === 'mpvLoadFile') {
        try {
          if (Module['loadFileDirect']) {
            Module['loadFileDirect'](msgData['path'], msgData['options'] || '');
          } else if (Module['loadFile']) {
            Module['loadFile'](msgData['path'], msgData['options'] || '');
          } else {
            throw new Error('libmpv module does not expose loadFileDirect/loadFile');
          }
          postMessage(JSON.stringify({ type: 'mpv-loadfile-dispatched', path: msgData['path'] }));
        } catch (ex) {
          postMessage(JSON.stringify({ type: 'mpv-loadfile-error', path: msgData['path'], error: String(ex && (ex.stack || ex.message) || ex) }));
        }
      } else if (cmd === 'mpvSetPause') {
        try {
          Module['setPause'](!!msgData['value']);
          postMessage(JSON.stringify({ type: 'mpv-command-dispatched', name: 'setPause', value: !!msgData['value'] }));
        } catch (ex) {
          postMessage(JSON.stringify({ type: 'mpv-command-error', name: 'setPause', error: String(ex && (ex.stack || ex.message) || ex) }));
        }
      } else if (cmd === 'mpvSetPlaybackTime') {
        try {
          Module['setPlaybackTime'](Number(msgData['seconds'] || 0));
          postMessage(JSON.stringify({ type: 'mpv-command-dispatched', name: 'setPlaybackTime', value: Number(msgData['seconds'] || 0) }));
        } catch (ex) {
          postMessage(JSON.stringify({ type: 'mpv-command-error', name: 'setPlaybackTime', error: String(ex && (ex.stack || ex.message) || ex) }));
        }
      } else if (cmd === 'mpvSetVolume') {
        try {
          Module['setVolume'](Number(msgData['value'] || 0));
          postMessage(JSON.stringify({ type: 'mpv-command-dispatched', name: 'setVolume', value: Number(msgData['value'] || 0) }));
        } catch (ex) {
          postMessage(JSON.stringify({ type: 'mpv-command-error', name: 'setVolume', error: String(ex && (ex.stack || ex.message) || ex) }));
        }
      } else if (cmd === 'mpvSetAudioTrack') {
        try {
          Module['setAudioTrack'](Number(msgData['id']));
          postMessage(JSON.stringify({ type: 'mpv-command-dispatched', name: 'setAudioTrack', value: Number(msgData['id']) }));
        } catch (ex) {
          postMessage(JSON.stringify({ type: 'mpv-command-error', name: 'setAudioTrack', error: String(ex && (ex.stack || ex.message) || ex) }));
        }
      } else if (cmd === 'mpvSetSubtitleTrack') {
        try {
          Module['setSubtitleTrack'](Number(msgData['id']));
          postMessage(JSON.stringify({ type: 'mpv-command-dispatched', name: 'setSubtitleTrack', value: Number(msgData['id']) }));
        } catch (ex) {
          postMessage(JSON.stringify({ type: 'mpv-command-error', name: 'setSubtitleTrack', error: String(ex && (ex.stack || ex.message) || ex) }));
        }
      } else if (cmd === 'run') {
        ...
        if (msgData['networkfsConfigs'] && NetworkFS && typeof NetworkFS.importConfigs === 'function') {
          try {
            NetworkFS.importConfigs(msgData['networkfsConfigs']);
          } catch (ex) {
            err('NetworkFS import failed: ' + String(ex && (ex.stack || ex.message) || ex));
          }
        }
...
var ASM_CONSTS = {
  5410860: ($0) => { postMessage(JSON.stringify({ type: 'idle', shaderCount: $0 })); },  
 5410928: () => { postMessage(JSON.stringify({ type: 'file-start' })); },  
 5410981: () => { postMessage(JSON.stringify({ type: 'file-end' })); },  
 5411032: ($0) => { postMessage(JSON.stringify({ type: 'property-change', name: UTF8ToString($0), value: 0 })); },  
 5411128: ($0, $1) => { postMessage(JSON.stringify({ type: 'property-change', name: UTF8ToString($0), value: UTF8ToString($1) })); },  
 5411239: ($0, $1) => { postMessage(JSON.stringify({ type: 'property-change', name: UTF8ToString($0), value: $1 })); },  
 5411336: ($0, $1) => { postMessage(JSON.stringify({ type: 'property-change', name: UTF8ToString($0), value: $1 })); },  
 5411433: ($0, $1) => { postMessage(JSON.stringify({ type: 'property-change', name: UTF8ToString($0), value: $1.toString() })); },  
 5411541: () => { arr = []; },  
 5411551: () => { arr.push(obj); },  
 5411566: () => { postMessage(JSON.stringify({ type: 'track-list', tracks: arr })); },  
 5411636: () => { postMessage(JSON.stringify({ type: 'chapter-list', chapters: arr })); },  
 5411710: () => { obj = {}; },  
 5411720: ($0, $1) => { obj[UTF8ToString($0)] = $1.toString(); },  
 5411763: ($0, $1) => { obj[UTF8ToString($0)] = UTF8ToString($1); },  
 5411809: ($0, $1) => { obj[UTF8ToString($0)] = $1; },  
 5411841: ($0, $1) => { obj[UTF8ToString($0)] = $1; },  
 5411873: ($0) => { if (!$0) { AL.alcErr = 0xA004 ; return 1; } },  
 5411921: ($0) => { if (!AL.currentCtx) { err("alGetProcAddress() called without a valid context"); return 1; } if (!$0) { AL.currentCtx.err = 0xA003 ; return 1; } },  
 5412069: ($0) => { var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return allocate(intArrayFromString(reply), 'i8', ALLOC_NORMAL); },  
 5412294: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 5412441: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
 5412675: ($0) => { if (typeof(Module['SDL3']) === 'undefined') { Module['SDL3'] = {}; } var SDL3 = Module['SDL3']; if (!$0) { SDL3.audio_playback = {}; } else { SDL3.audio_recording = {}; } if (!SDL3.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL3.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL3.audioContext = new webkitAudioContext(); } if (SDL3.audioContext) { if ((typeof navigator.userActivation) === 'undefined') { autoResumeAudioContext(SDL3.audioContext); } } } return SDL3.audioContext === undefined ? -1 : 0; },  
 5413245: () => { return Module['SDL3'].audioContext.sampleRate; },  
 5413296: ($0, $1, $2, $3) => { var SDL3 = Module['SDL3']; var have_microphone = function(stream) { if (SDL3.audio_recording.silenceTimer !== undefined) { clearInterval(SDL3.audio_recording.silenceTimer); SDL3.audio_recording.silenceTimer = undefined; SDL3.audio_recording.silenceBuffer = undefined } SDL3.audio_recording.mediaStreamNode = SDL3.audioContext.createMediaStreamSource(stream); SDL3.audio_recording.scriptProcessorNode = SDL3.audioContext.createScriptProcessor($1, $0, 1); SDL3.audio_recording.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) { if ((SDL3 === undefined) || (SDL3.audio_recording === undefined)) { return; } audioProcessingEvent.outputBuffer.getChannelData(0).fill(0.0); SDL3.audio_recording.currentRecordingBuffer = audioProcessingEvent.inputBuffer; dynCall('vi', $2, [$3]); }; SDL3.audio_recording.mediaStreamNode.connect(SDL3.audio_recording.scriptProcessorNode); SDL3.audio_recording.scriptProcessorNode.connect(SDL3.audioContext.destination); SDL3.audio_recording.stream = stream; }; var no_microphone = function(error) { }; SDL3.audio_recording.silenceBuffer = SDL3.audioContext.createBuffer($0, $1, SDL3.audioContext.sampleRate); SDL3.audio_recording.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { SDL3.audio_recording.currentRecordingBuffer = SDL3.audio_recording.silenceBuffer; dynCall('vi', $2, [$3]); }; SDL3.audio_recording.silenceTimer = setInterval(silence_callback, ($1 / SDL3.audioContext.sampleRate) * 1000); if ((navigator.mediaDevices !== undefined) && (navigator.mediaDevices.getUserMedia !== undefined)) { navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(have_microphone).catch(no_microphone); } else if (navigator.webkitGetUserMedia !== undefined) { navigator.webkitGetUserMedia({ audio: true, video: false }, have_microphone, no_microphone); } },  
 5415137: ($0, $1, $2, $3) => { var SDL3 = Module['SDL3']; SDL3.audio_playback.scriptProcessorNode = SDL3.audioContext['createScriptProcessor']($1, 0, $0); SDL3.audio_playback.scriptProcessorNode['onaudioprocess'] = function (e) { if ((SDL3 === undefined) || (SDL3.audio_playback === undefined)) { return; } if (SDL3.audio_playback.silenceTimer !== undefined) { clearInterval(SDL3.audio_playback.silenceTimer); SDL3.audio_playback.silenceTimer = undefined; SDL3.audio_playback.silenceBuffer = undefined; } SDL3.audio_playback.currentPlaybackBuffer = e['outputBuffer']; dynCall('vi', $2, [$3]); }; SDL3.audio_playback.scriptProcessorNode['connect'](SDL3.audioContext['destination']); if (SDL3.audioContext.state === 'suspended') { SDL3.audio_playback.silenceBuffer = SDL3.audioContext.createBuffer($0, $1, SDL3.audioContext.sampleRate); SDL3.audio_playback.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { if ((typeof navigator.userActivation) !== 'undefined') { if (navigator.userActivation.hasBeenActive) { SDL3.audioContext.resume(); } } SDL3.audio_playback.currentPlaybackBuffer = SDL3.audio_playback.silenceBuffer; dynCall('vi', $2, [$3]); SDL3.audio_playback.currentPlaybackBuffer = undefined; }; SDL3.audio_playback.silenceTimer = setInterval(silence_callback, ($1 / SDL3.audioContext.sampleRate) * 1000); } },  
 5416453: ($0) => { var SDL3 = Module['SDL3']; if ($0) { if (SDL3.audio_recording.silenceTimer !== undefined) { clearInterval(SDL3.audio_recording.silenceTimer); } if (SDL3.audio_recording.stream !== undefined) { var tracks = SDL3.audio_recording.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL3.audio_recording.stream.removeTrack(tracks[i]); } } if (SDL3.audio_recording.scriptProcessorNode !== undefined) { SDL3.audio_recording.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL3.audio_recording.scriptProcessorNode.disconnect(); } if (SDL3.audio_recording.mediaStreamNode !== undefined) { SDL3.audio_recording.mediaStreamNode.disconnect(); } SDL3.audio_recording = undefined; } else { if (SDL3.audio_playback.scriptProcessorNode != undefined) { SDL3.audio_playback.scriptProcessorNode.disconnect(); } if (SDL3.audio_playback.silenceTimer !== undefined) { clearInterval(SDL3.audio_playback.silenceTimer); } SDL3.audio_playback = undefined; } if ((SDL3.audioContext !== undefined) && (SDL3.audio_playback === undefined) && (SDL3.audio_recording === undefined)) { SDL3.audioContext.close(); SDL3.audioContext = undefined; } },  
 5417609: ($0, $1) => { var SDL3 = Module['SDL3']; var numChannels = SDL3.audio_playback.currentPlaybackBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL3.audio_playback.currentPlaybackBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio playback buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[$0 + ((j*numChannels + c) << 2) >> 2]; } } },  
 5418113: ($0, $1) => { var SDL3 = Module['SDL3']; var numChannels = SDL3.audio_recording.currentRecordingBuffer.numberOfChannels; for (var c = 0; c < numChannels; ++c) { var channelData = SDL3.audio_recording.currentRecordingBuffer.getChannelData(c); if (channelData.length != $1) { throw 'Web Audio recording buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } if (numChannels == 1) { for (var j = 0; j < $1; ++j) { setValue($0 + (j * 4), channelData[j], 'float'); } } else { for (var j = 0; j < $1; ++j) { setValue($0 + (((j * numChannels) + c) * 4), channelData[j], 'float'); } } } },  
 5418740: () => { if (typeof(Module['SDL3']) === 'undefined') { Module['SDL3'] = {}; } Module['SDL3'].camera = {}; },  
 5418841: () => { return (navigator.mediaDevices === undefined) ? 0 : 1; },  
...
// end include: preamble.js
  ...
  var spawnThread = (threadParams) => {
      ...
      if (typeof NetworkFS != 'undefined' && NetworkFS && typeof NetworkFS.exportConfigs == 'function') {
        try {
          msg.networkfsConfigs = NetworkFS.exportConfigs();
        } catch (_) {
        }
      }
  ...
  var ___pthread_create_js = (pthread_ptr, attr, startRoutine, arg) => {
      ...
      // postMessage.
      if (false && ENVIRONMENT_IS_PTHREAD && (transferList.length === 0 || error)) {
  ...
  var wasmfsExternalFSBlobs = new HandleAllocator();
  var __wasmfs_externalfs_close_blob = (blobID) => {
      wasmfsExternalFSBlobs.free(blobID);
    };

  var wasmfsExternalFSDirectoryHandles = new HandleAllocator();
  var __wasmfs_externalfs_free_directory = (dirID) => {
      wasmfsExternalFSDirectoryHandles.free(dirID);
    };

  var wasmfsExternalFSFileHandles = new HandleAllocator();
  var __wasmfs_externalfs_free_file = (fileID) => {
      wasmfsExternalFSFileHandles.free(fileID);
    };

  
  async function wasmfsExternalFSGetOrCreateFile(parent, name, create) {
  ...
  async function wasmfsExternalFSGetOrCreateDir(parent, name, create) {
      let parentHandle = wasmfsExternalFSDirectoryHandles.get(parent);
      let childHandle;
      try {
  ...
  var wasmfsExternalFSProxyFinish = (ctx) => {
  ...
  async function __wasmfs_externalfs_get_child(ctx, parent, namePtr, childTypePtr, childIDPtr) {
  ...
  async function __wasmfs_externalfs_get_entries(ctx, dirID, entriesPtr, errPtr) {
  ...
  var __wasmfs_externalfs_get_size_blob = (blobID) => {
      // This cannot fail.
  ...
  async function __wasmfs_externalfs_get_size_file(ctx, fileID, sizePtr) {
  ...
  var __wasmfs_externalfs_get_object_store = async function() {
  ...
  var __wasmfs_externalfs_key_exists = async function(store, key) {
      return new Promise((resolve, reject) => {
  ...
  var __wasmfs_externalfs_get_key = async function(store, key) {
  ...
  async function __wasmfs_externalfs_init_root_directory(ctx, directoryNamePtr, successPtr) {
  ...
  async function __wasmfs_externalfs_open_blob(ctx, fileID, blobIDPtr) {
      let fileHandle = wasmfsExternalFSFileHandles.get(fileID);
      let blobID;
      try {
  ...
  async function __wasmfs_externalfs_read_blob(ctx, blobID, bufPtr, len, pos, nreadPtr) {
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(5, 0, 1, bufferId, param, value0, value1, value2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(6, 0, 1, bufferId, param, value0, value1, value2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(7, 0, 1, bufferId, format, pData, size, freq);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(8, 0, 1, bufferId, param, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(9, 0, 1, bufferId, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(10, 0, 1, bufferId, param, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(11, 0, 1, bufferId, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(12, 0, 1, count, pBufferIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(14, 0, 1, sourceId, param, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(13, 0, 1, count, pSourceIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(15, 0, 1, param);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(16, 0, 1, model);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(17, 0, 1, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(18, 0, 1, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(19, 0, 1, param);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(20, 0, 1, count, pBufferIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(21, 0, 1, count, pSourceIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(22, 0, 1, param);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(23, 0, 1, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(24, 0, 1, bufferId, param, pValue0, pValue1, pValue2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(25, 0, 1, bufferId, param, pValue0, pValue1, pValue2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(26, 0, 1, bufferId, param, pValue);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(27, 0, 1, bufferId, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(28, 0, 1, bufferId, param, pValue);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(29, 0, 1, bufferId, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(30, 0, 1, param);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(31, 0, 1, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(32, 0, 1, pEnumName);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(34, 0, 1, param);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(35, 0, 1, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(36, 0, 1, param);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(37, 0, 1, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(38, 0, 1, param, pValue0, pValue1, pValue2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(39, 0, 1, param, pValue0, pValue1, pValue2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(40, 0, 1, param, pValue);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(41, 0, 1, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(42, 0, 1, param, pValue);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(43, 0, 1, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(44, 0, 1, sourceId, param, pValue0, pValue1, pValue2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(45, 0, 1, sourceId, param, pValue0, pValue1, pValue2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(46, 0, 1, sourceId, param, pValue);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(47, 0, 1, sourceId, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(48, 0, 1, sourceId, param, pValue);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(49, 0, 1, sourceId, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(50, 0, 1, param);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(51, 0, 1, bufferId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(52, 0, 1, param);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(53, 0, 1, pExtName);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(54, 0, 1, sourceId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(55, 0, 1, param, value0, value1, value2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(56, 0, 1, param, value0, value1, value2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(57, 0, 1, param, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(58, 0, 1, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(59, 0, 1, param, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(60, 0, 1, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(61, 0, 1, sourceId, param, value0, value1, value2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(62, 0, 1, sourceId, param, value0, value1, value2);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(63, 0, 1, sourceId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(64, 0, 1, count, pSourceIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(65, 0, 1, sourceId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(66, 0, 1, count, pSourceIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(67, 0, 1, sourceId, count, pBufferIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(68, 0, 1, sourceId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(69, 0, 1, count, pSourceIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(70, 0, 1, sourceId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(71, 0, 1, count, pSourceIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(72, 0, 1, sourceId, count, pBufferIds);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(73, 0, 1, sourceId, param, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(74, 0, 1, sourceId, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(75, 0, 1, sourceId, param, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(76, 0, 1, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(77, 0, 1, deviceId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(78, 0, 1, deviceId, pAttrList);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(79, 0, 1, contextId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(80, 0, 1, contextId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(81, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(82, 0, 1, deviceId, pEnumName);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(83, 0, 1, deviceId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(84, 0, 1, deviceId, param, size, pValues);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(85, 0, 1, contextId);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(86, 0, 1, pDeviceName);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(87, 0, 1, api);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(88, 0, 1, display, attrib_list, configs, config_size, numConfigs);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(89, 0, 1, display, config, hmm, contextAttribs);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(90, 0, 1, display, context);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(91, 0, 1, display, config, attribute, value);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(92, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(93, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(94, 0, 1, nativeDisplayType);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(95, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(96, 0, 1, display, majorVersion, minorVersion);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(97, 0, 1, display, draw, read, context);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(98, 0, 1, display, name);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(99, 0, 1, display, interval);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(101, 0, 1, target, width, height);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(100, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(102, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(103, 0, 1, status);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(104, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(105, 0, 1, target, width, height);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(106, 0, 1, index, gamepadState);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(107, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(108, 0, 1, width, height);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(109, 0, 1, target, deferUntilInEventHandler, fullscreenStrategy);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(111, 0, 1);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(112, 0, 1, userData, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(114, 0, 1, target, width, height);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(115, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(116, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(117, 0, 1, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(118, 0, 1, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(119, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(120, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(121, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(122, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(123, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(124, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(125, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(126, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(127, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(128, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(129, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(130, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(131, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(132, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(133, 0, 1, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(134, 0, 1, target, userData, useCapture, callbackfunc, targetThread);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(135, 0, 1, title);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(136, 0, 1, __environ, environ_buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(137, 0, 1, penviron_count, penviron_buf_size);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(138, 0, 1, node, service, hint, out);
  ...
  var __wasmfs_externalfs_delete_key = async function(store, key) {
      return new Promise((resolve, reject) => {
  ...
  var NetworkFS = (() => {
      ...
      function request(method, url, headers, range) {
        var xhr = new XMLHttpRequest();
      ...
      function responseBytes(xhr) {
        ...
        for (var i = 0; i < text.length; i++) {
          data[i] = text.charCodeAt(i) & 255;
        }
        return data;
      }

      function probeSize(url, headers) {
        if (String(url).indexOf('/mpvwasm/proxy') >= 0) {
          var rangeXhr = request('GET', url, headers, 'bytes=0-0');
          if (rangeXhr.status === 206) {
            var directRangeSize = parseContentRange(rangeXhr.getResponseHeader('Content-Range'));
            if (directRangeSize > 0) return directRangeSize;
          }
          throw new Error('NetworkFS cannot determine proxy file size, status=' + rangeXhr.status);
        }

        var xhr = request('HEAD', url, headers);
        if (xhr.status >= 200 && xhr.status < 400) {
          var contentRangeSize = parseContentRange(xhr.getResponseHeader('Content-Range'));
          if (contentRangeSize > 0) return contentRangeSize;

          var length = Number(xhr.getResponseHeader('Content-Length') || 0);
          if (length > 0) return length;
        }

        if (String(url).indexOf('/mpvwasm/proxy') < 0) {
          throw new Error('NetworkFS cannot determine file size for non-proxy URL');
        }

        xhr = request('GET', url, headers, 'bytes=0-0');
        if (xhr.status === 206) {
          var rangeSize = parseContentRange(xhr.getResponseHeader('Content-Range'));
          if (rangeSize > 0) return rangeSize;
        }

        throw new Error('NetworkFS cannot determine file size, status=' + xhr.status);
      }

      function readRange(state, start, end) {
        var xhr = request('GET', state.url, state.headers, 'bytes=' + start + '-' + (end - 1));
        if (xhr.status !== 206 && xhr.status !== 200) {
          throw new Error('NetworkFS range request failed, status=' + xhr.status);
        }
        var data = responseBytes(xhr);
        if (xhr.status === 200 && data.length > end - start) {
          throw new Error('NetworkFS upstream ignored Range');
        }
        return data;
      }

      function getChunk(state, index) {
        var cached = state.cache.get(index);
        if (cached) {
        ...
        var data = readRange(state, start, end);
        state.cache.set(index, data);
        state.cacheBytes += data.length;

        while (state.cacheBytes > state.maxCacheBytes && state.cache.size > 1) {
          var firstKey = state.cache.keys().next().value;
          var first = state.cache.get(firstKey);
          state.cache.delete(firstKey);
          state.cacheBytes -= first ? first.length : 0;
        }

        return data;
      }

      function read(state, buffer, length, offset) {
        if (typeof offset === 'bigint') offset = Number(offset);
        buffer = Number(buffer);
        length = Number(length);
        if (!Number.isFinite(offset) || offset < 0) return -22;
        if (!Number.isFinite(buffer) || !Number.isFinite(length) || length < 0) return -22;
        if (offset >= state.size) return 0;

        var remaining = Math.min(length, state.size - offset);
        var copied = 0;

        while (remaining > 0) {
          var absolute = offset + copied;
          var chunkIndex = Math.floor(absolute / state.chunkSize);
          var chunkOffset = absolute - chunkIndex * state.chunkSize;
          var chunk = getChunk(state, chunkIndex);
          var take = Math.min(remaining, chunk.length - chunkOffset);
          if (take <= 0) break;
          HEAPU8.set(chunk.subarray(chunkOffset, chunkOffset + take), buffer + copied);
          copied += take;
          remaining -= take;
        }
      ...
      function createState(path, url, opts) {
        opts = opts || {};
        var state = {
          url,
          headers: normalizeHeaders(opts.headers),
          size: Number(opts.size || 0),
          chunkSize: Number(opts.chunkSize || 1048576),
          maxCacheBytes: Number(opts.maxCacheBytes || 33554432),
          cacheBytes: 0,
          cache: new Map()
        };
        if (!(state.size > 0)) state.size = probeSize(url, state.headers);
        return state;
      }

      function registerBackend(state, backendPointer) {
        wasmFS$backends[backendPointer] = {
          ...
          read: (file, buffer, length, offset) => {
            try {
              ...
              err('NetworkFS read failed:', e && (e.stack || e.message) || e);
              return -29;
            }
          },
          write: () => -28
      ...
      function finishMount(path, state, backendPointer, attached) {
        var id = sequence++;
        state.id = id;
        state.path = path;
        state.backendPointer = backendPointer;
        mounts[path] = state;
        configs[backendPointer] = {
          path,
          url: state.url,
        ...
        return { id, path, size: state.size, chunkSize: state.chunkSize, backendPointer, attached: !!attached };
      }

      function mountFile(path, url, opts) {
        opts = opts || {};
        if (!path || path[0] !== '/') path = '/' + (path || ('net/source-' + sequence + '.bin'));

        var parent = path.slice(0, path.lastIndexOf('/')) || '/';
        try {
          FS.mkdirTree(parent, 511);
        } catch (e) {
        }

        try {
          if (FS.findObject(path)) FS.unlink(path);
        } catch (e) {
        }

        var state = createState(path, url, opts);
        var backendPointer = _wasmfs_create_jsimpl_backend();
        registerBackend(state, backendPointer);

        FS.handleError(withStackSave(() =>
          _wasmfs_create_file(stringToUTF8OnStack(path), FS_getMode(true, false), backendPointer)
        ));

        return finishMount(path, state, backendPointer, false);
      }

      function attachFile(path, url, opts) {
        opts = opts || {};
        if (!path || path[0] !== '/') path = '/' + (path || ('net/source-' + sequence + '.bin'));
        var backendPointer = Number(opts.backendPointer || 0);
        if (!backendPointer) throw new Error('NetworkFS attach requires backendPointer');
        var state = createState(path, url, opts);
        registerBackend(state, backendPointer);
        return finishMount(path, state, backendPointer, true);
      }
      ...
      function importConfigs(list) {
        var imported = 0;
        (list || []).forEach((config) => {
          if (!config || !config.backendPointer || configs[config.backendPointer]) return;
          attachFile(config.path, config.url, config);
          imported++;
        });
        return imported;
      }
      ...
      function readTest(path, length, offset) {
        length = Number(length || 16);
        offset = Number(offset || 0);
        var stream = FS.open(path, 'r');
        try {
          var buffer = new Uint8Array(length);
          var nread = FS.read(stream, buffer, 0, length, BigInt(offset));
          var bytes = Array.prototype.slice.call(buffer, 0, nread);
          var hex = bytes.map((b) => ('0' + b.toString(16)).slice(-2)).join(' ');
          return { nread, hex };
        } finally {
          FS.close(stream);
        }
...
function assignWasmImports() {
  wasmImports = {
    ...
    /** @export */
    _wasmfs_externalfs_close_blob: __wasmfs_externalfs_close_blob,
    /** @export */
    _wasmfs_externalfs_free_directory: __wasmfs_externalfs_free_directory,
    /** @export */
    _wasmfs_externalfs_free_file: __wasmfs_externalfs_free_file,
    /** @export */
    _wasmfs_externalfs_get_child: __wasmfs_externalfs_get_child,
    /** @export */
    _wasmfs_externalfs_get_entries: __wasmfs_externalfs_get_entries,
    /** @export */
    _wasmfs_externalfs_get_size_blob: __wasmfs_externalfs_get_size_blob,
    /** @export */
    _wasmfs_externalfs_get_size_file: __wasmfs_externalfs_get_size_file,
    /** @export */
    _wasmfs_externalfs_init_root_directory: __wasmfs_externalfs_init_root_directory,
    /** @export */
    _wasmfs_externalfs_open_blob: __wasmfs_externalfs_open_blob,
    /** @export */
    _wasmfs_externalfs_read_blob: __wasmfs_externalfs_read_blob,
    /** @export */
...
var _wasmfs_create_externalfs_backend = createExportWrapper('wasmfs_create_externalfs_backend', 1);
...
var __wasmfs_externalfs_record_entry = createExportWrapper('_wasmfs_externalfs_record_entry', 3);
...
var _ff_h264_cabac_tables = Module['_ff_h264_cabac_tables'] = 2665788;
...
Module['ExternalFS'] = ExternalFS;
Module['NetworkFS'] = NetworkFS;
var missingLibrarySymbols = [
  ...
  'wasmfsFetchFSProxyFinish',
  'wasmfsFetchFSGetOrCreateFile',
  'wasmfsFetchFSGetOrCreateDir',
  'writeGLArray',
...
var unexportedSymbols = [
  ...
  'wasmfsOPFSBlobs',
  'wasmfsExternalFSDirectoryHandles',
  'wasmfsExternalFSFileHandles',
  'wasmfsExternalFSAccessHandles',
  'wasmfsExternalFSBlobs',
  'wasmfsExternalFSProxyFinish',
  'wasmfsExternalFSGetOrCreateFile',
  'wasmfsExternalFSGetOrCreateDir',
  'wasmfsFetchFSHandles',
  'wasmfsFetchFSBlobs',
